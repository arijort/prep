...
def color_graph(graph, colors):
    ...
    # iterate over graph as list
    for node in graph:
    # get colors from neighbors which already have a color
        if node in node.neighbors:
            raise Exception
        illegal_colors = [ n.color for n in node.neighbors if n.color ]
        for c in colors:
    # find a color not in set of neighbors
